...
def move_prop_over(paint, acr, thickness):
    wave = paint[:, 0]
    prop = zeros((5, int(len(wave))))
    prop[0, :] = acr[:, 1]
    prop[1, :] = paint[:, 1]*10000
    prop[2, :] = paint[:, 2]*10000
    prop[3, :] = paint[:, 3]*paint[:, 2]*10000
    prop[4, :] = thickness
    return prop



def mie_theory(r1, fv1, paint, acr, thickness, dist, particle_type):
    # if the input is the scattering properties instead of n and k, just move them over
    if particle_type == 1:
