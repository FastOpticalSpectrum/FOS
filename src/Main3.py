...
def optical(line, infile, particle, medium, check, p_num, m_num):
    ...
    for i in range(line+1, len(infile)):
        ...
        if infile[i][0:5] == "upper":
            ...
            # get the index for each material number (index 1 may refer to particle 2 if they are not sequentially numbered)
            mtype_in = get_index(mtype, 'm', p_num, m_num)
            optics_sum = effective_medium(optics_sum, vol_frac_sum, medium[int(mtype_in), :, :])
    ...
    for i in range(sims):
        ...
        with open(output_sim, 'w') as f:
            ...
            for layer in range(num_layers):
                f.write('\tn(' + str(layer+1) + ')' + '\tmu_a(' + str(layer+1) + ') 1/cm'+ '\tmu_s(' + str(layer+1) + ') 1/cm'+ '\tg(' + str(layer+1) + ')'+ '\tt(' + str(layer+1) + ') cm')
            f.write('\n')
            for j in range(length):
                ...
                for layer in range(num_layers):
                    ...
                    f.write(str(round(prop[prop_line - (1+num_layers) + layer, 1], 4)) + '\t')
                    f.write(str(round(prop[prop_line - (1+num_layers) + layer, 2], 4)) + '\t')
                    f.write(str(round(prop[prop_line - (1+num_layers) + layer, 3], 4)) + '\t')
                    f.write(str(round(prop[prop_line - (1+num_layers) + layer, 4], 4)) + '\t')
                prop_line += 1
                f.write('\n')
            f.write('\n')
            f.write('Input file:\n')
...
if __name__ == "__main__":
    ...
    print('{: ^75s}'.format("Version: 0.4.1\n"))
